...
var SpaceService = (function () {
    function SpaceService(http) {
        this.http = http;
        this.url = '/api/space/'; // URL to web API
    }
    SpaceService.prototype.getSpace = function (id) {
    ...
    SpaceService.prototype.updateSpace = function (space) {
        ...
        var body = JSON.stringify(space);
        return this.http.put(this.url + space.Id, body, options)
            .map(this.extractData)
            .catch(this.handleError);
    };
    SpaceService.prototype.extractData = function (res) {
        var body = res.json();
        return body || {};
    };
    SpaceService.prototype.handleError = function (error) {
